...
contract AMMPair is IAMMPair, AMMERC20 {
  ...
  function swap(
    ...
    require(swapPaused == false, 'AMM: Swap paused');
    ...
    require(amount0In > 0 || amount1In > 0, 'AMM: INSUFFICIENT_INPUT_AMOUNT');
    {
      ...
      uint256 balance0Adjusted = (balance0.mul(10000).sub(amount0In.mul(txFee)));
      uint256 balance1Adjusted = (balance1.mul(10000).sub(amount1In.mul(txFee)));
      require(balance0Adjusted.mul(balance1Adjusted) >= uint256(_reserve0).mul(_reserve1).mul(10000**2), 'AMM: K');
    }
  ...
  function setTxFee(uint256 fee) external {
    require(msg.sender == factory, 'AMM: Unauthorized sender trying to set tx fee');
    txFee = fee;
  }

  /* Pause swap */
  function pauseSwap() external {
    require(msg.sender == factory, 'AMM: Unauthorized sender trying to pause');
    swapPaused = true;
  }

  /* Resume swap */
  function resumeSwap() external {
    require(msg.sender == factory, 'AMM: Unauthorized sender trying to resume');
    swapPaused = false;
  }
}
